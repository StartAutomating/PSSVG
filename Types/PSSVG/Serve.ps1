...
if ($hasCache.Value) {
    ...
    if ($hasCache.Value -is [int]) {
        ...
        $response.Headers["Cache-Control"] = "public, max-age=$(60 * 60 * 24 * 7)"
        # Return the SVG
        $showSplat = $this.Resolve($request, $this.ExportedCommands['Show-SVG'])
        return ($hasCache.Value | Show-SVG @showSplat)
    }    
}
...
if (($resolvedRequest -is [Collections.IDictionary]) -and 
    ...
    if ($cmdOut -as [xml]) {
        ...
        return ($cmdOut | Show-SVG @showSplat)
    }
    elseif ($cmdOut -is [xml.xmlelement]) {            
        $this.RequestCache[$cacheKey] = $cmdOut        
        return ($this.RequestCache[$cacheKey] | Show-SVG @showSplat)
    }
    elseif ($cmdOut -as [IO.FileInfo]) {
        $svgFileInfo = $cmdOut
        if ($svgFileInfo.Extension -eq '.svg') {
            $cmdOut = [IO.File]::ReadAllText($svgFileInfo.FullName) | Show-SVG @showSplat
            $this.RequestCache[$cacheKey] = $cmdOut
            return $cmdOut
        }
        return ""
...
elseif ($resolvedRequest -is [IO.FileInfo]) {
    $svgFileInfo = $resolvedRequest
    if ($svgFileInfo.Extension -eq '.svg') {
        $toCache = [IO.File]::ReadAllText($svgFileInfo.FullName) | FrameSVG
        $this.RequestCache[$cacheKey] = $toCache
        return $toCache
    }
    elseif ($svgFileInfo.Extension -in '.md', '.markdown') {
        $toCache = SVG @(
            $markdownContent = [IO.File]::ReadAllText($svgFileInfo.FullName)
